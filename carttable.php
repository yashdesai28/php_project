...
if (isset($_POST['cdelete'])) {
 ...
 $deletewi="DELETE FROM `cart` WHERE cid='$cid'";
...
if(isset($_POST['upcart'])){
	...
	print_r($_POST['sque']);
	echo"stotal";
	print_r($_POST['stotal']);

	$cout=0;

	foreach ($_POST['ucid'] as $val){
	...
	for($i=0;$i<$cout;$i++){
		...
		$ucid=$_POST['ucid'][$i];
		$stotal=$_POST['stotal'][$i];
		$sque=$_POST['sque'][$i];

		 

		$upstor="UPDATE `cart` SET `que`='$sque',`total`='$stotal' WHERE  cid='$ucid'";
		mysqli_query($conn,$upstor);
...
#name, #price, #total, #remove, #quntity {
    height: 9rem;
    width: 100%;
...
<body>
		...
		<div class="container2">
			...
			<table class="table">
            	...
            	while ($row = mysqli_fetch_array($result)) {
					...
					$product = "SELECT * FROM `tblproduct` WHERE pname='$row[3]' ";
					$rang = mysqli_query($conn, $product);
					$rangque = mysqli_fetch_array($rang);
					


					echo'<tr>
						<td id="remove" data-label="remove">';
					...
					echo'<button class="btn-productr" name=cdelete ><svg xmlns="http://www.w3.org/2000/svg"  class="iconremove"  fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
						...
						echo'</td>';
						echo'<td id="product_image" data-label="product image">';  
						echo "<img ";
						echo 'src="./images/';
						echo "$row[2]";
						echo '">';
				  echo'</td>';
						...
						echo'<td id="name" data-label="name">'.$row[3].'</td>';
						
						echo'<td id="price" data-label="price">'.$row[4].' &#8377<input type="hidden"  class="price1" id="price1"  value="'.$row[4].'" /></td>';
						echo'<td   id="quntity" data-label="ountity">';

							
							echo'<input type="number" class="que"  onchange="subtotal()" min=1 max='.$rangque[6].' name="" value="'.$row[5].'">';

							echo" <input type=hidden class=sque name=sque[] value=" . $row[5] . "  >";
						...
						echo'<td class="total" id="total" data-label="total">';
					...
					echo"<input type=hidden class=stotal name=stotal[] value=" . $row[0] . "  >";

					echo" <input type=hidden name=ucid[] value=" . $row[0] . "  >";
		...
		<div class="main-cart">
			<div class="cart-left">
				<div class="two-btn">
					...
					<a class="cart-btn"><button type="submit" class="up-cart" >Update Cart</button></a>

				
					<form action="" method="POST">
					<a class="cart-btn" href="product.html"><button class="up-cart" name="productshow">Countinue Shopping</button></a>
					</form>
				</div>
			...
			<div class="cart-right">
				<table class="table1">
					...
					<tbody>
						...
						<tr>
							...
							<td id="grandtotal" data-label="Name"></td>
				...
				<!-- <a class="cart-btn" href="placeorder.php"><button class="up-cart">Proceed To Cheakout</button></a> -->
				<button type="submit"  name="upcart" class="up-cart">Proceed To Cheakout</button>
	...
	<script>
		...
		var sque=document.getElementsByClassName('sque');
		...
		function subtotal(){
			...
			for(i=0;i<price.length;i++){
				...
				total[i].innerHTML=(price[i].value)*(que[i].value);
				stotal[i].value=(price[i].value)*(que[i].value);
				sque[i].value=que[i].value;
				gt+=(price[i].value)*(que[i].value);
